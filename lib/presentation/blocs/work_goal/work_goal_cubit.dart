...
part 'work_goal_state.dart';

class WorkGoalCubit extends Cubit<WorkGoalState> {
  final WatchWorkEntriesUseCase _watchEntries;
  final WorkGoalRemoteStore _remoteStore;
  ...
  WorkGoalCubit(
    ...
    Stream<List<ConnectivityResult>>? connectivityChanges,
        ...
        super(const WorkGoalState()) {
    _connectivitySub = _connectivityChanges.listen(_onConnectivityChanged);
  }

  Future<void> setUser(String userId) async {
    if (state.userId == userId && state.status == WorkGoalStatus.ready) {
      return;
    }
  ...
  Future<void> setGoalForMonth(DateTime month, Duration goal) async {
    ...
    final nextUpdatedAtByMonth = Map<String, DateTime>.from(state.updatedAtByMonth);
  ...
  Future<WorkGoalSnapshot?> _loadPendingSnapshot(String userId) async {
    final prefs = _prefs ??= await SharedPreferences.getInstance();
  ...
  Future<void> _replaceLocalSnapshotCache(
    ...
    final minutesPrefix = '${AppConstants.workGoalMinutesKeyPrefix}:$userId:';
    final updatedAtPrefix = '${AppConstants.workGoalUpdatedAtKeyPrefix}:$userId:';
  ...
  Map<String, int> _loadGoalMinutesByMonth(
    ...
    final prefix = '${AppConstants.workGoalMinutesKeyPrefix}:$userId:';
  ...
  Map<String, DateTime> _loadUpdatedAtByMonth(
    ...
    final prefix = '${AppConstants.workGoalUpdatedAtKeyPrefix}:$userId:';
  ...
  static String _prefKey(String userId, DateTime date) =>
      '${AppConstants.workGoalMinutesKeyPrefix}:$userId:${_monthKey(date)}';

  static String _pendingPrefKey(String userId) =>
      '${AppConstants.workGoalPendingKeyPrefix}:$userId';
